  ...
  try {
    ...
    if (!session) {
      return NextResponse.json(
        { error: "No live session. Ask the agent for real-time data first." },
        { status: 404 }
      )
    }
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        for (const m of (await getRecentMarketEvents(chainId, 15)).reverse()) {
          send(m)
        }
        cleanup = await subscribeChannels([marketChannel(chainId)], (_c, m) =>
          send(m)
        )
